...
public class EntrustMQServer {
    ...
    @Autowired
    private TradeClientIfac tadeClientIfac;

    @Autowired
    FinanceWalletService financeWalletService;
            ...
            for (MarketWebSocket mws : socketsEntrust) {
                if (mws.getSession() != null) {
                    ...
                    if (userIdList.contains(userId)) {
                        ResultJson<TradeEntrustVO> resultJson = tadeClientIfac.userEntrustListWebsocket(userId, symbol, type, status);
                        ZipUtil.gzAndSendWesocket(mws.getSession(), resultJson);
                    }
                }
            }
